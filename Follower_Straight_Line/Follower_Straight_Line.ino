...
void loop() {
  ...
  if (state == STATE_RUNNING_EXPERIMENT) {
    ...
    // offset = abs(leftBump - rightBump);

    // change = millis() - pid_test_ts;
    // if (change >= 500) {

    //   bumpError[i] = leftBump - rightBump;
    //   i++;
    //   pid_test_ts = millis();
    // }
    ...
    if (average != 0) {
      ...
      correct = follow.update(0.0, difference);
      last_difference = difference;

      correct = correct * 0.00007;

      Serial.println(correct);

      // float forward_vel;
      // forward_vel = 2;
      // forward_vel = correct * forward_vel;
